...
<div class="w-full h-full inline-block flex flex-row" style="--color: {color}; --fullDuration: {fullDuration}">
    {#each [...Array(blocks).keys()].map((i) => i + 1) as version}
		<div
			class="rect grow-[3]"
			class:pause-animation={pause}
			style="animation-delay: {(version - 1) * (+duration / (12*flow))}{durationUnit}"
		/>
        {#if version !== blocks}
        <div class="grow"></div>
        {/if}
